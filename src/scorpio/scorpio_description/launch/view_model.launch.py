...
def generate_launch_description():
    available_urdf_files = [f for f in os.listdir(os.path.join(get_package_share_directory('scorpio_description'), 'urdf')) if f.startswith('scorpio')]
    ...
    if ('model' not in params or params['model'] not in available_urdf_files):
        print ('USAGE:')
        print ('ros2 launch scorpio_description view_model.launch.py model:=<model>')
    ...
    xacro_path = os.path.join(get_package_share_directory('scorpio_description'), 'urdf', params['model'])
