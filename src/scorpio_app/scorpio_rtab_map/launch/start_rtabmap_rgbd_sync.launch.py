...
def generate_launch_description():
    ...
    scorpio_rtab_map_dir = get_package_share_directory('scorpio_rtab_map')
    scorpio_bringup_dir = get_package_share_directory('scorpio_bringup')
    ...
    rviz_config_dir = os.path.join(get_package_share_directory('scorpio_rtab_map'),
                                   'rviz', 'scorpio_rtabmap.rviz')
    declare_params_file = DeclareLaunchArgument(
            'params_file',
            default_value=os.path.join(
            get_package_share_directory('scorpio_navigation2'),
            'param',
            'scorpio_navigation.yaml'),
    ...
    declare_rviz_config = DeclareLaunchArgument(# -----------修改---------------
        'rviz_config', 
        default_value='scorpio_base.rviz',
    ...
    driver_bringup_group = GroupAction([
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(scorpio_bringup_dir, 'launch',
    ...
    map_nav_group = GroupAction([
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(scorpio_rtab_map_dir, 'launch',
                                                       'scorpio_rtabmap_rgbd_sync.launch.py')),
    ...
    scorpio_teleop_node = launch_ros.actions.Node(
        package='scorpio_teleop',
    ...
    scorpio_map_nav_rviz_node = launch_ros.actions.Node(
        package='rviz2',
        executable='rviz2',
        name='rviz2',
    ...
    scorpio_delay_map_nav_action = TimerAction(period=5.0, actions=[map_nav_group])
    #scorpio_delay_save_map_action = TimerAction(period=10.0, actions=[scorpio_save_map_node])
    ...
    ld.add_action(scorpio_teleop_node)
    ld.add_action(scorpio_map_nav_rviz_node)
    ld.add_action(scorpio_delay_map_nav_action)
    #ld.add_action(scorpio_delay_save_map_action)
    return ld
