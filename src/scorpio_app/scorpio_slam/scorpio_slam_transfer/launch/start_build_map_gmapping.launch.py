...
def generate_launch_description():
    ...
    scorpio_slam_dir = get_package_share_directory('slam_gmapping')
    ...
    rviz_config_dir = os.path.join(get_package_share_directory('scorpio_slam_transfer'),
                                   'rviz', 'scorpio_slam.rviz')
    ...
    driver_bringup_group = GroupAction([
        ...
        # IncludeLaunchDescription(
        #     PythonLaunchDescriptionSource(os.path.join(scorpio_slam_dir, 'launch',
    ...
    scorpio_save_map_node = launch_ros.actions.Node(
        package='scorpio_slam_transfer',
        executable='save_map_cmd.sh',  
        output='screen',
        emulate_tty=True,
        )

    scorpio_slam_rviz_node = launch_ros.actions.Node(
        package='rviz2',
        executable='rviz2',
        name='rviz2',
    ...
    scorpio_delay_slam_action = TimerAction(period=1.0, actions=[slam_group])
    scorpio_delay_save_map_action = TimerAction(period=1.0, actions=[scorpio_save_map_node])
    ...
    ld.add_action(scorpio_slam_rviz_node)
    ld.add_action(scorpio_delay_slam_action)
