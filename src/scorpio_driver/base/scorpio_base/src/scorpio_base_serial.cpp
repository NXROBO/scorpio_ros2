...
namespace NxScorpioBase
{
    ...
    ScorpioSerial::ScorpioSerial(std::string dev_name, uint32_t baudrate = 115200) : serial_name_(dev_name),
                                                                                     baudrate_(baudrate)
    {
    }

    ScorpioSerial::~ScorpioSerial()
    {
        if (fd_ != -1)
        {
            CloseSerial();
    ...
    int ScorpioSerial::OpenSerial()
    {
        int i;
        int speed_arr[] = {B1500000, B1000000, B460800, B230400, B115200, B57600, B19200, B9600, B4800, B2400, B1200, B300};
        unsigned int name_arr[] = {1500000, 1000000, 460800, 230400, 115200, 57600, 19200, 9600, 4800, 2400, 1200, 300};
        fd_ = -1;
    ...
    void ScorpioSerial::hex_printf(unsigned char *buf, int len)
    {
#if 1
        int i;
        for (i = 0; i < len; i++)
    ...
    int ScorpioSerial::WriteBuffer(uint8_t *buf, uint16_t length)
    {
        return write(fd_, buf, length);
    }

    int ScorpioSerial::GetDataGram(unsigned char *r_buffer, int *length)
    {
        int fs_sel;
        fd_set fs_read;
